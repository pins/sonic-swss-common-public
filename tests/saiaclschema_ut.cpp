...
INSTANTIATE_TEST_CASE_P(
    SaiAclSchemaTest, FormatTest,
    testing::Values(Format::kNone, Format::kHexString, Format::kMac,
                    Format::kIPv4, Format::kIPv6, Format::kString),
...
INSTANTIATE_TEST_CASE_P(
    SaiAclSchemaTest, StageTest,
    testing::Values(Stage::kLookup, Stage::kIngress, Stage::kEgress),
...
TEST(SaiAclSchemaTest, MatchFieldSchemaByNameSucceeds) {
  EXPECT_THAT(MatchFieldSchemaByName("SAI_ACL_TABLE_ATTR_FIELD_SRC_IPV6"),
              AllOf(Field(&MatchFieldSchema::stages,
                          UnorderedElementsAre(Stage::kLookup, Stage::kIngress,
                                               Stage::kEgress)),
                    Field(&MatchFieldSchema::format, Format::kIPv6),
                    Field(&MatchFieldSchema::bitwidth, 128)));
}
...
TEST(SaiAclSchemaTest, ActionSchemaByNameSucceeds) {
  EXPECT_THAT(ActionSchemaByName("SAI_ACL_ENTRY_ATTR_ACTION_SET_INNER_VLAN_ID"),
              AllOf(Field(&ActionSchema::format, Format::kHexString),
                    Field(&ActionSchema::bitwidth, 12)));
}
