...
ConsumerNotifier::ConsumerNotifier(const std::string &notifier_channel_name,
                                   DBConnector *db_connector) {
  notifier_channel_name_ = std::string{notifier_channel_name};
  notification_consumer_ = std::make_unique<NotificationConsumer>(
      db_connector, notifier_channel_name_);
}

bool ConsumerNotifier::WaitForNotificationAndPop(
    std::string &op, std::string &data, std::vector<FieldValueTuple> &values,
    int64_t timeout_ms) {
