...
Stage StageFromName(const std::string& name) {
  if (name == kStageLookupName) return Stage::kLookup;
  if (name == kStageIngressName) return Stage::kIngress;
  if (name == kStageEgressName) return Stage::kEgress;
...
const std::string& StageName(Stage stage) {
  ...
  throw std::invalid_argument(std::string(__func__) +
                              " called with an invalid enum value.");
}

Format FormatFromName(const std::string& name) {
  if (name == kFormatNone) return Format::kNone;
  if (name == kFormatHexString) return Format::kHexString;
  if (name == kFormatMac) return Format::kMac;
  if (name == kFormatIPv4) return Format::kIPv4;
  if (name == kFormatIPv6) return Format::kIPv6;
  if (name == kFormatString) return Format::kString;
...
const MatchFieldSchema& MatchFieldSchemaByName(
    const std::string& match_field_name) {
  static const auto* const kMatchFields =
      new std::unordered_map<std::string, MatchFieldSchema>({
          {"SAI_ACL_TABLE_ATTR_FIELD_SRC_IPV6",
           {.stages = {Stage::kLookup, Stage::kIngress, Stage::kEgress},
            .format = Format::kIPv6,
            .bitwidth = 128}},
          ...
          {"SAI_ACL_TABLE_ATTR_FIELD_IN_PORT",
           {.stages = {Stage::kLookup, Stage::kIngress, Stage::kEgress},
            .format = Format::kString,
            .bitwidth = 0}},
          {"SAI_ACL_TABLE_ATTR_FIELD_OUT_PORT",
           {.stages = {Stage::kIngress, Stage::kEgress},
            .format = Format::kString,
            .bitwidth = 0}},
  ...
  static const auto* const kActions =
      new std::unordered_map<std::string, ActionSchema>({
          ...
          {"SAI_ACL_ENTRY_ATTR_ACTION_SET_SRC_MAC",
           {.format = Format::kMac, .bitwidth = 48}},
          ...
          {"SAI_ACL_ENTRY_ATTR_ACTION_SET_DSCP",
           {.format = Format::kHexString, .bitwidth = 6}},
          {"SAI_ACL_ENTRY_ATTR_ACTION_SET_ECN",
           {.format = Format::kHexString, .bitwidth = 2}},
